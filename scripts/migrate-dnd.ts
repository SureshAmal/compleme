...
async function run() {
  try {
    ...
    await db.query(
      `ALTER TABLE topics ADD COLUMN IF NOT EXISTS position DOUBLE PRECISION DEFAULT 0;`,
    );
    await db.query(
      `ALTER TABLE todos ADD COLUMN IF NOT EXISTS position DOUBLE PRECISION DEFAULT 0;`,
    );
