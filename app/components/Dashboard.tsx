...
const TOPIC_COLORS = [
  ...
  {
    accent: "#AC8E68",
    bg: "rgba(172, 142, 104, 0.08)",
    border: "rgba(172, 142, 104, 0.25)",
  },
];
      ...
      <div className="todo-tooltip-content">
        ...
        {completedAt && (
          <div className="todo-tooltip-row">
            ...
            <span className="todo-tooltip-value">
...
export default function Dashboard({
  ...
  const positionTooltip = (wrapper: HTMLElement) => {
    ...
    const tooltipContent = wrapper.querySelector(
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = (e: MouseEvent) => {
      ...
      if (!textWrapper) return;
      const tooltip = textWrapper.querySelector(
        ".todo-tooltip-wrapper",
      ) as HTMLElement;
      if (tooltip) {
  ...
  const handleToggleTodo = async (todoId: number, is_completed: boolean) => {
    ...
    setTodos(
      todos.map((t) =>
        t.id === todoId
          ? { ...t, is_completed, completed_at: newCompletedAt }
          : t,
      ),
    );
      ...
      if (catTopics.length === 0) {
        ...
        catTopics.forEach((topic: Topic) => {
          const topicTodos = todos.filter(
            (td: Todo) => td.topic_id === topic.id,
          );
          if (topicTodos.length === 0) {
            exportRows.push(
              [escape(cat.name), escape(topic.name), "", ""].join(","),
            );
          } else {
  ...
  const activeTopics = topics
    .filter((t: Topic) => t.category_id === activeCategoryId)
  ...
  const topicStats = activeTopics.map((topic: Topic, idx: number) => {
    ...
    return {
      name: topic.name,
      percentage,
      fill: TOPIC_COLORS[idx % TOPIC_COLORS.length].accent,
    };
  });

  const allActiveTodos = todos.filter((td: Todo) =>
    activeTopics.some((t: Topic) => t.id === td.topic_id),
  );
  const totalActive = allActiveTodos.length;
  const totalActiveCompleted = allActiveTodos.filter(
    (td: Todo) => td.is_completed,
  ).length;
  const pieData =
    totalActive > 0
      ? [
          { name: "Completed", value: totalActiveCompleted },
          { name: "Incomplete", value: totalActive - totalActiveCompleted },
        ]
          ...
          <div key={cat.id} className="tab-wrapper">
            ...
            <button
              ...
              onClick={() =>
                openDeleteConfirm(
                  "Delete Category",
                  `Are you sure you want to delete "${cat.name}" and all its topics and todos?`,
                  () => {
          ...
          {(provided) => (
            ...
            >
              {activeTopics.map((topic: Topic, idx: number) => {
                ...
                const topicTodos = todos
                  .filter((td: Todo) => td.topic_id === topic.id)
                  .sort((a, b) => a.position - b.position);
                return (
                  <Draggable
                    ...
                    {(provided) => (
                      ...
                      >
                        ...
                        >
                          ...
                          />
                          <button
                            className="topic-delete hover-child"
                            onClick={() =>
                              openDeleteConfirm(
                                "Delete Topic",
                                `Delete "${topic.name}" and all its todos?`,
                                ...
                                >
                                  {(provided) => (
                                    ...
                                    >
                                      ...
                                      <button
                                        className="todo-delete hover-child"
                                        onClick={() =>
                                          openDeleteConfirm(
                                            "Delete Todo",
                                            `Delete "${todo.text}"?`,
                        ...
                        <div className="add-todo-form hover-add-target">
                          <input
                            ...
                            onChange={(e) =>
                              setNewTodoValue((prev) => ({
                                ...prev,
                                [topic.id]: e.target.value,
                              }))
                            }
                            onKeyDown={(e) =>
                              e.key === "Enter" && handleAddTodo(topic.id)
                            }
                          ...
                          <button
                            ...
                            onClick={() => handleAddTodo(topic.id)}
                            disabled={addingTodoTo === topic.id}
                          >
                            {addingTodoTo === topic.id ? (
                              <div
                                className="spinner"
                                style={{
                                  width: 16,
                  ...
                  <button
                    ...
                    onClick={() =>
                      setModalConfig({
                        ...
                        onSubmit: handleAddTopic.bind(null, activeCategoryId),
                      })
                    }
        ...
        <div className="analytics-header-section">
          <div>
            ...
            <p className="analytics-subtitle">
              {totalActiveCompleted} of {totalActive} tasks completed
            </p>
          </div>
          <div className="global-progress-circle">
            <svg viewBox="0 0 36 36" className="circular-chart">
              ...
              <text x="18" y="20.35" className="percentage">
                {totalActive > 0
                  ? Math.round((totalActiveCompleted / totalActive) * 100)
                  : 0}
                %
              </text>
            </svg>
          ...
          {topicStats.map((stat: any, idx: number) => (
            <div key={idx} className="topic-progress-row">
              <div className="topic-progress-label">
                <ChevronRight
                  size={14}
                  style={{ color: stat.fill, marginRight: "4px" }}
                />
                <span>{stat.name}</span>
              </div>
